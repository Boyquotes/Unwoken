...
onready var Health_bar = get_node("Health_bar_tank")
...
var reset_timer_finished = true

func _on_ResetTimer_timeout():
	detected = false
	reset_timer_finished = true
	...
	for pos in [target_position, nw, se, ne, sw]:
		...
		if result:
			if result.collider.name == "Player":
				var aim_speed_r = deg2rad(2)
	
				if $Head.get_angle_to(target_position) > 0:
					$Head.rotation += aim_speed_r
				if $Head.get_angle_to(target_position) < 0:
					$Head.rotation -= aim_speed_r
				...
				if reset_timer_finished and detected:  
					reset_timer.start()
					reset_timer_finished = false
				else:
					if detected == false:
						...
						if(direction == 1):
							...
							$Head_collision.set_rotation_degrees(0)
						else:
							$Head.flip_v = true
							$Head.set_rotation_degrees(180)
							$Head_collision.set_rotation_degrees(180)
...
func shoot(pos):
	if can_shoot: # ha lőhet
		...
		laser.shoot(pos, $Head/Barrel1.get_global_position())
		# timer inditása
		can_shoot = false
		shoot_timer.start()
